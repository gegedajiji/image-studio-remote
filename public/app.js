...
function cleanCommunityDisplayText(text) {
  return String(text || '')
    .replace(/\s*清晰度要求[:：][^\n\r。；;]*/gi, '')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
...
function inferCommunityTags(prompt, item = {}) {
...
function communityPublishDraft(item = {}) {
...
function normalizeCommunityPostForDisplay(post) {
  if (!post) return post;
